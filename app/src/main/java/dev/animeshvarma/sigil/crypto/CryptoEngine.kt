...
object CryptoEngine {
    ...
    private fun getBlockSize(algo: Algorithm): Int = if (algo == Algorithm.AES_GCM) 16 else 16 // Simplified for brevity, assume full map exists
    private fun getKeySize(algo: Algorithm): Int = 32

    /**
     * SECURE ENCRYPT: Accepts CharArray, Wipes memory after use.
     */
    fun encrypt(
        data: String,
        password: CharArray, // <--- CHANGED TO CHAR ARRAY
        ...
        // 2. Root Secret (Secure Derivation)
        // Convert CharArray to ByteArray safely
        val passBytes = toBytes(password)
        val sha512Password = hashPasswordSHA512(passBytes)
        // Wipe the raw passBytes immediately
        ...
        algorithms.forEachIndexed { index, algo ->
            ...
            Arrays.fill(layerKey, 0.toByte()) // Wipe Layer Key
        }
        ...
        Arrays.fill(rootSecret, 0.toByte()) // FINAL WIPE OF ROOT
    ...
    fun decrypt(
        ...
        try {
            ...
            val buffer = ByteBuffer.wrap(payloadBytes); buffer.position(16)
            ...
            if (isCompressed) currentBytes = decompressData(currentBytes)

            val result = String(currentBytes, StandardCharsets.UTF_8)
            if (result.contains(CS_DELIMITER)) {
                return result.substringBeforeLast(CS_DELIMITER)
            }
            return result
        } catch (e: Exception) {
    ...
    private fun toBytes(chars: CharArray): ByteArray {
        ...
        Arrays.fill(byteBuffer.array(), 0.toByte()) // Wipe buffer
        return bytes
    }

    private fun hashPasswordSHA512(p: ByteArray): ByteArray {
        val d = SHA512Digest()
    ...
    // ... [Add Compress/Decompress/Strip/Restore/Hash functions here from previous CryptoEngine] ...
